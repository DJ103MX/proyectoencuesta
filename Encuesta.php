...
class Poll{
    ...
    public function addVote($subjectOption)
    {
       ...
       $sql = "INSERT INTO tbl_preg_votos (course_id) VALUES ('".$subjectOption."')";
    ...
    {
        $daoObject = new encuestas();
        $sql = "SELECT * FROM tbl_preg order by id";
    ...
    public function getCourcewithVote()
    {
        ...
        if(!empty($result)) {
            foreach ($result as $k => $v) {
                ...
                $sql_stmt = "SELECT COUNT(course_id) as vote_count FROM tbl_preg_votos WHERE course_id = ". $result[$k]["id"];
